...
void renderizaInterfaz(){
    ...
    char cC1[] = {0xCC,0xCC,0xCC,255}; // Color de contorno
    // --Funciones
    ...
    // Dibuja la recta mediante algoritmo de Bresenham

    // Dibuja circunferencia
    void dibujarCircun(int x0, int y0, int radio, char color[]){
        int x = radio;
        int y = 0;
        ...
        int radioError = 0;
        SDL_SetRenderDrawColor(renderizador,color[0],color[1],color[2],color[3]);
    while (x >= y) {     //Dibuja circunferencia

                SDL_RenderDrawPoint(renderizador,x0 - x, y0 + y);
                SDL_RenderDrawPoint(renderizador,x0 - x, y0 - y); 
                SDL_RenderDrawPoint(renderizador,x0 + x, y0 + y);
                SDL_RenderDrawPoint(renderizador,x0 + x, y0 - y); 

                SDL_RenderDrawPoint(renderizador, x0 - y, y0 + x);
                SDL_RenderDrawPoint(renderizador, x0 - y, y0 - x);
                SDL_RenderDrawPoint(renderizador, x0 + y, y0 + x);
                SDL_RenderDrawPoint(renderizador, x0 + y, y0 - x); 
                y++;
    ...
    // Dibuja circulo
    void dibujarCircul(int x0, int y0, int radio, char color[]){
        int x = radio;
        int y = 0;
        int xCambio = 1 - (radio << 1);
        int yCambio = 0;
    ...
    while (x >= y) {     //Dibuja circunferencia
            for (int i = x0 - x; i <= x0 + x; i++) {
                SDL_RenderDrawPoint(renderizador,i, y0 + y);
                SDL_RenderDrawPoint(renderizador,i, y0 - y); }
            for (int i = x0 - y; i <= x0 + y; i++) {
                SDL_RenderDrawPoint(renderizador,i, y0 + x);
                SDL_RenderDrawPoint(renderizador,i, y0 - x); }
                y++;
                radioError += yCambio;
                yCambio += 2;
    ...
    // amperimetro
    //--Dibuja la cuarta circunferencia
    void dibujar4Circun(int x0, int y0, int radio, char color[]){
        int x = radio;
        int y = 0;
        int xCambio = 1 - (radio << 1);
        int yCambio = 0;
    ...
    while (x >= y) {     //Dibuja circunferencia
                ...
                // SDL_RenderDrawPoint(renderizador,x0 - x, y0 + y);
                // SDL_RenderDrawPoint(renderizador,x0 - x, y0 - y); //cu
                // SDL_RenderDrawPoint(renderizador,x0 + x, y0 + y);
                // SDL_RenderDrawPoint(renderizador,x0 + x, y0 - y); 
                ...
                SDL_RenderDrawPoint(renderizador, x0 + y, y0 - x); 
                // if(){}
                SDL_RenderDrawLine(renderizador, x0, y0 , x0- y, y0 -x);
                SDL_RenderDrawLine(renderizador, x0, y0 , x0+ y, y0 -x);
    ...
    dibujarCircun(20 + anchoVentana/8 - 15,60 + altoVentana/4 + 0,30,cC0);
    // SDL_RenderDrawLine(renderizador,0,0,anchoVentana, altoVentana);
    dibujar4Circun (95 + anchoVentana/4 - 15,60 + altoVentana/4 + 0,100,cC1);
